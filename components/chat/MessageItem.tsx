...
export default function MessageItem({ message, isCurrentUser }: MessageItemProps) {
	return (
		<div className={`flex ${isCurrentUser ? "justify-end" : "justify-start"}`}>
			<div
				className={`max-w-xs md:max-w-md lg:max-w-lg xl:max-w-xl rounded-2xl px-3.5 py-2.5 shadow-sm shadow-slate-900/50 ${
					isCurrentUser
						? "bg-gradient-to-br from-sky-500 to-indigo-500 text-slate-50 rounded-br-sm"
			...
			>
				...
				<div
					className={`mt-1 flex items-center gap-1 text-[0.7rem] ${
						isCurrentUser ? "text-sky-100/90" : "text-slate-400"
					}`}
				>
					<span>
					...
					{isCurrentUser && (
						<span className="ml-0.5">
							{message.read ? <DoubleTick color="#4ade80" /> : <DoubleTick />}
						</span>
					)}
				</div>
			</div>
